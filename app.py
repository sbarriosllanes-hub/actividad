...
# Define the base path for model files. For Streamlit Cloud deployment, these should be in the same directory as app.py
ia_dir_deployment = '.' # Changed for deployment
...
def load_models():
    try:
        ...
        pipeline = joblib.load(os.path.join(ia_dir_deployment, 'pipeline_preproc.pkl'))
        model = joblib.load(os.path.join(ia_dir_deployment, 'best_knn_model.pkl'))
        return pipeline, model
    except Exception as e:
        st.error(f"Error al cargar los modelos: {e}. Asegúrate de que 'pipeline_preproc.pkl' y 'best_knn_model.pkl' estén en el mismo directorio que app.py en tu repositorio.")
        st.stop()
